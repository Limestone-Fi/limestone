...
import {IDromeCallee} from "../../interfaces/external/IDromeCallee.sol";
import {MockPoolFees} from "./MockDromePoolFees.sol";
...
interface IPoolCallee is IDromeCallee {}

/// @title Pool
/// @author velodrome.finance, @figs999, @pegahcarter
/// @notice Veldrome V2 token pool, either stable or volatile
contract MockPool is ERC20Permit, ReentrancyGuard {
    ...
    IDromePool.Observation[] public observations;
    ...
    function initialize(address _token0, address _token1, bool _stable) external {
        if (factory != address(0)) revert IDromePool.FactoryAlreadySet();
        factory = msg.sender;
        (token0, token1, stable) = (_token0, _token1, _stable);
        poolFees = address(new MockPoolFees(_token0, _token1));
        string memory symbol0 = "USDC";
        string memory symbol1 = "AERO";
        if (_stable) {
        ...
        observations.push(IDromePool.Observation(block.timestamp, 0, 0));
    }
    ...
    function mutateSupply(uint256 _newSupply) external {
        _mint(address(1), _newSupply);
    }
    ...
    function lastObservation() public view returns (IDromePool.Observation memory) {
    ...
    function claimFees() external returns (uint256 claimed0, uint256 claimed1) {
        ...
        if (claimed0 > 0 || claimed1 > 0) {
            ...
            MockPoolFees(poolFees).claimFeesFor(msg.sender, claimed0, claimed1);

            emit IDromePool.Claim(msg.sender, msg.sender, claimed0, claimed1);
        }
    }
    ...
    function _update0(uint256 amount) internal {
        ...
        emit IDromePool.Fees(msg.sender, amount, 0);
    }
    ...
    function _update1(uint256 amount) internal {
        ...
        emit IDromePool.Fees(msg.sender, 0, amount);
    }
    ...
    function _update(uint256 balance0, uint256 balance1, uint256 _reserve0, uint256 _reserve1) internal {
        ...
        IDromePool.Observation memory _point = lastObservation();
        timeElapsed = blockTimestamp - _point.timestamp; // compare the last observation with current timestamp, if greater than 30 minutes, record a new event
        if (timeElapsed > periodSize) {
            observations.push(IDromePool.Observation(blockTimestamp, reserve0CumulativeLast, reserve1CumulativeLast));
        }
        reserve0 = balance0;
        reserve1 = balance1;
        blockTimestampLast = blockTimestamp;
        emit IDromePool.Sync(reserve0, reserve1);
    }
        ...
        if (_totalSupply == 0) {
            ...
            if (stable) {
                if ((_amount0 * 1e18) / decimals0 != (_amount1 * 1e18) / decimals1) {
                    revert IDromePool.DepositsNotEqual();
                }
                if (_k(_amount0, _amount1) <= MINIMUM_K) revert IDromePool.BelowMinimumK();
            }
        } else {
            liquidity =
                FixedPointMathLib.min((_amount0 * _totalSupply) / _reserve0, (_amount1 * _totalSupply) / _reserve1);
        }
        if (liquidity < MINIMUM_LIQUIDITY) revert IDromePool.InsufficientLiquidityMinted();
        _mint(to, liquidity);

        _update(_balance0, _balance1, _reserve0, _reserve1);
        emit IDromePool.Mint(msg.sender, _amount0, _amount1);
    }
    ...
    function burn(address to) external nonReentrant returns (uint256 amount0, uint256 amount1) {
        ...
        if (amount0 == 0 || amount1 == 0) revert IDromePool.InsufficientLiquidityBurned();
        ...
        _update(_balance0, _balance1, _reserve0, _reserve1);
        emit IDromePool.Burn(msg.sender, to, amount0, amount1);
    }

    function swap(uint256 amount0Out, uint256 amount1Out, address to, bytes calldata data) external nonReentrant {
        if (IPoolFactory(factory).isPaused()) revert IDromePool.IsPaused();
        if (amount0Out == 0 && amount1Out == 0) revert IDromePool.InsufficientOutputAmount();
        (uint256 _reserve0, uint256 _reserve1) = (reserve0, reserve1);
        if (amount0Out >= _reserve0 || amount1Out >= _reserve1) revert IDromePool.InsufficientLiquidity();
        ...
        uint256 _balance1;
        {
            ...
            (address _token0, address _token1) = (token0, token1);
            if (to == _token0 || to == _token1) revert IDromePool.InvalidTo();
        ...
        if (amount0In == 0 && amount1In == 0) revert IDromePool.InsufficientInputAmount();
        {
            ...
            if (_k(_balance0, _balance1) < _k(_reserve0, _reserve1)) revert IDromePool.K();
        }

        _update(_balance0, _balance1, _reserve0, _reserve1);
        emit IDromePool.Swap(msg.sender, to, amount0In, amount1In, amount0Out, amount1Out);
    }
    ...
    function sync() external nonReentrant {
        if (totalSupply() == 0) revert IDromePool.InsufficientLiquidity();
